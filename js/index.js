/**
 * Created by jiaoge on 2017/9/27.
 */
    ...
    $("#php1")[0].style.left=($("#php").width()-20)+"px";
    $("#html1")[0].style.left=($("#html").width()-20)+"px";
    $("#ps1")[0].style.left=($("#ps").width()-20)+"px";
    $("#node1")[0].style.left=($("#node").width()-20)+"px";
    $("#js1")[0].style.left=($("#js").width()-20)+"px";
    console.log("js"+$("#js").width());
    $("#jq1")[0].style.left=($("#jq").width()-20)+"px";
    $(".li").on("click",function () {
       $(this).addClass("active").siblings().removeClass("active");
       $(".div").eq($(this).index()).addClass("divActive").siblings().removeClass("divActive");
    });
...
//position.left就是读取不能设置
// =($("#jq").width()-25)+"px";

$("a").on("click",function () {
    var bool=$("#two").is(':checked');
    // var bool=$("#two").attr("checked");
    // var bool=$("#two").is(':checked');
    console.log(bool);
    if(bool)
    {
        // $("#a").attr('href','myBUTTON.html');
        setTimeout(function () {
            $(window).attr('location','myBUTTON.html');
        },1000);
    //为什么用点击label时候判断选中状态是相反的但是改成最外面的父元素a判断就对了
    }});
